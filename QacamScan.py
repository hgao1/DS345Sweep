...
import logging
logging.basicConfig()
logger = logging.getLogger('QacamScan')
...
class QacamScan(QObject):
    ...
    -------
    newData(object)
        Emitted for every data point.
        Value is a list of polargraph indexes,
        polargraph position and lockin data.
    sweepData(object)
        Emitted for every data point. 
        Value is a list of lockin data, 
        and function generator frequency. 
    motion(object)
    ...
    Sweeprun()
        Perform a frequency sweep scan with data acquisition. 
        Then return to home position and release motors. 
    '''

    newData = pyqtSignal(object)
    sweepData= pyqtSignal(object) 
    motion = pyqtSignal(object)
    finished = pyqtSignal(bool)
    freqData = pyqtSignal(object) 
    ...
    def Sweeprun(self):
        ...
        source.mute = False
        source._On = True 
        source._trigger= True
        time.sleep(10) #sleep for 10 seconds
        while not self._reset:
            self.sweepData.emit([lockin.data])
